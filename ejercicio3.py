...
def funcion_obj(Model):
    return sum(instalation_cost[l] * Model.x[s, l] for s in Model.S for l in Model.L) + \
           sum((energy_consumption[s] + communication_cost.get((s,l),0)) * Model.x[s, l] for s in Model.S for l in Model.L)

Model.obj = Objective(rule=funcion_obj, sense=minimize)

# Restricción: Cada ubicación debe estar cubierta por al menos un sensor
def coverage_rule(Model, l):
    return sum(Model.x[s, l] * sensor_coverage[(s,l)] for s in Model.S) >= 1

Model.coverage_constraint = Constraint(Model.L, rule=coverage_rule)
